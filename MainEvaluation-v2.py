...
from EvaluationDebateLoop_debug import LiveDebateOrchestration
import json
import pickle
...
def load_embedded_model_configs(config_file_path: str):
    ...
    for section_name in section_candidates:
        ...
        # Normalize: wrap plain dicts in a list so everything is List[dict]
        normalized = {}
        for model_name, cfg in section.items():
            if isinstance(cfg, dict):
                # single config — infer run_name from script name
...
def get_models_from_path(path, embedded_model_configs):
    ...
    for file in sorted(folder.glob("*.py")):
        ...
        for model_cfg in embedded_model_configs[module_name]:  # iterate over runs
