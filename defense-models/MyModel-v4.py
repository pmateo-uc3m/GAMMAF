...
class KMeansCluster:
    def __init__(self, device, config, n_benign_clusters=1, scaler=None):
    ...
    def classify_embeddings(self, embeddings):
        ...
        if self.scaler is not None:
            standardized = self.scaler.transform(flat_emb)
        else:
...
class WindowBreakerModel:
        ...
        with torch.no_grad():
            for idx in indices:
                ...
                for round_id in unique_rounds:
                    ...
                    flat = node_points_cloud.cpu().numpy().reshape(-1, node_points_cloud.shape[-1])
    ...
    def _train(self, train_data_loader):
        ...
        scaler_fit_indices = []
        if has_val and val_indices:
            scaler_fit_indices.extend(val_indices)
        if has_thresh_val and thresh_val_indices:
            scaler_fit_indices.extend(thresh_val_indices)

        if scaler_fit_indices:
            self.fitted_scaler = self._fit_standard_scaler(train_data_loader, scaler_fit_indices)
        else:
    ...
    def predict(self, graph_data, adj_matrix, flags_ground_truth=None):
        per_agent = [
        ...
        # import os
        # log_path = os.path.join(os.path.dirname(__file__), "MyModel-v3-debug-output.txt")
