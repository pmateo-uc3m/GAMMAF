...
live_evaluation_config:
  ...
  # Optional: path to the MS MARCO contaminated benchmark JSON generated by
  # MA/Task_generation/main.py. Only used when questions_dataset_tag is MA.
...
### MS MARCO memory-poisoning benchmark

The MA dataset (`TAG = "MA"`, loader `MSMARCOLoader`) consumes a contaminated
benchmark generated by `MA/Task_generation/main.py`. Each entry carries the
original query, the correct answer, the original ("safe") passages, and the
LLM-contaminated ("adversarial") passages:
...
`MSMARCOLoader` maps these to the debate prompt placeholders:

* benign agents receive the safe passages via `{safe_texts}`,
