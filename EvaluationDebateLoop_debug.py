...
def post_results(url, flags_gt, flags, scores, embeddings, adjacency_matrix, answers, question_gt, clusters, model_id, raw_text=None):
    ...
    # embeddings → binary
    emb_buf = io.BytesIO()
    np.save(emb_buf, embeddings)
    emb_buf.seek(0)
    files["embeddings"] = ("embeddings.npy", emb_buf, "application/octet-stream")

    # scores → binary
    scores_buf = io.BytesIO()
    np.save(scores_buf, scores)
    scores_buf.seek(0)
    files["scores"] = ("scores.npy", scores_buf, "application/octet-stream")

    # FIX: ensure adjacency_matrix is JSON-serializable
    ...
    try:
        requests.post(
            url,
            data={"meta": json.dumps(data), "model_id": model_id},
            files=files,
            timeout=5,
        )
    except Exception:
...
class LiveDebateOrchestration:
    ...
    def debate_question(
        ...
        model_name = defense_model.config.model_name if defense_model.config.model_name else defense_model.__class__.__module__
        
        log_embeddings = "XG" not in model_name
        ...
        # POST to data writer server flags_ground_truth, flags, anomaly_scores, embedding
        # Save answers for debugging
        ...
        for i in range(1, self.config.max_rounds):
            ...
            # POST to data writer server flags_ground_truth, flags, anomaly_scores, embeddings
